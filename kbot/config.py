...
#: Below this, a derived key is not worth deriving — say so instead.
MIN_MASTER_KEY_CHARS = 24


def resolve_master_key(raw: str) -> str:
    """Turn whatever the operator supplied into a usable Fernet key.

    A proper Fernet key is used exactly as given. Anything else long enough is
    hashed into one deterministically, so a secret generated by a host's own
    "generate a random value" feature works — which is what makes one-click
    deploys possible without asking someone to run a command first.

    The mapping is stable: the same input always yields the same key, so
    credentials encrypted under it stay readable across restarts and redeploys.
    """
    import base64
    import hashlib
    ...
    if len(candidate) < MIN_MASTER_KEY_CHARS:
        raise ConfigError(
            f"MASTER_KEY is too short ({len(candidate)} characters). Use at "
            f"least {MIN_MASTER_KEY_CHARS} random characters, or generate a "
            "proper key with: python -m kbot.tools genkey"
        )
